...
conditions.  This thesis bridges that gap with an open-source,
Docker-based FL simulator that combines a real FedAvg training loop
(ResNet-18 on CIFAR-10, non-IID Dirichlet $\alpha = 0.5$) with
kernel-level network emulation via Linux \texttt{tc netem}.

A systematic QoS parameter sweep across five bandwidth levels (1--100
Mbps), three latency values (5--200~ms), and three packet loss rates
(0--5\%) was conducted over 20 FL rounds with 3 clients, supplemented by
scaling experiments (5 and 7 clients) and a straggler scenario.  The
results identify a throughput floor of $\beta^* \approx 3$~Mbps, below
which communication time exceeds training time and multi-round
convergence becomes impractical.  Crucially, bandwidth does not affect
per-round model accuracy: all configurations completing 18+ rounds
converged to 65--67\% average accuracy, confirming that the effect of
constrained bandwidth is purely on training speed, not learning quality.
Packet loss at 5\% caused complete experiment failure, while latency up
to 200~ms had negligible impact ($<$3\% increase in round duration).

These findings yield a practical client admission criterion
($\beta_k \geq 3$~Mbps, $\rho_k < 5\%$) applicable to real-world FL
deployments, and demonstrate that the straggler effect---where one slow
...
5 & N/A (failed) & 42.0 & $\gg 1\times$ & Experiment failed at round~5 \\
\bottomrule
\end{tabular}
\end{table}
...
At 5\% loss, the experiment \textbf{failed}: client-3 timed out waiting
for round completion after round~5.  The \texttt{submit\_ms} at 5\%
loss was $\approx$42\,s per round (vs.\ 3.9\,s at 0\%), a $10\times$
increase driven by exponential TCP back-off under sustained loss.  This
demonstrates that even moderate packet loss can render synchronous FL
inoperable when combined with large model payloads, as retransmission
cascades push round durations beyond timeout thresholds.
...
\paragraph{RQ3 (Minimum Network Requirement).}  A practical client
admission rule can be stated as: exclude any client with
$\beta_k < 3$~Mbps or $\rho_k \geq 5\%$.  Latency up to 200\,ms has
negligible impact and need not be gated.  This simple threshold protects
round duration without sacrificing accuracy, since the excluded clients
would contribute the same per-round learning as any other client but
at dramatically higher time cost.
