...
def train_server(fx_client, y, l_epoch_count, l_epoch, idx, len_batch):
    ...
    if count1 == len_batch:
        ...
        if len(idx_collect) == num_users:
            fed_check = (
                True  # to evaluate_server function  - to check fed check has hitted
            )
...
class Client(object):
    def __init__(
        ...
        self.ldr_train = DataLoader(
            DatasetSplit(dataset_train, idxs), batch_size=256, shuffle=True
        )
        self.ldr_test = DataLoader(
            DatasetSplit(dataset_test, idxs_test), batch_size=256, shuffle=True
        )
