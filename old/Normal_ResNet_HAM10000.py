...
for epoch in range(epochs):
    train_loss, train_acc = train(
        net_glob, device, train_iterator, optimizer, criterion
    )
    ...
    prRed(
        f"Train => Epoch: {epoch} \t Acc: {train_acc*100:05.2f}% \t Loss: {train_loss:.3f}"
    )
    prGreen(
        f"Test =>               \t Acc: {test_acc*100:05.2f}% \t Loss: {test_loss:.3f}"
    )
