...
print(
    f"---------{program}----------"
)  # this is to identify the program in the slurm outputs files
...
class ResNet18_client_side(nn.Module):
    ...
    def forward(self, x):
        ...
        out1 = (
            out1 + resudial1
        )  # adding the resudial inputs -- downsampling not required in this layer
...
if torch.cuda.device_count() > 1:
    ...
    net_glob_client = nn.DataParallel(
        net_glob_client
    )  # to use the multiple GPUs; later we can change this to CPUs only
...
def train_server(fx_client, y, l_epoch_count, l_epoch, idx, len_batch):
    ...
    count1 += 1
    if count1 == len_batch:
        acc_avg_train = sum(batch_acc_train) / len(
            batch_acc_train
        ...
        prRed(
            "Client{} Train => Local Epoch: {} \tAcc: {:.3f} \tLoss: {:.4f}".format(
        ...
        if l_epoch_count == l_epoch - 1:
            ...
            l_epoch_check = (
                True  # for evaluate_server function - to check local epoch has hitted
            )
        ...
        if len(idx_collect) == num_users:
            fed_check = (
                True  # for evaluate_server function  - to check fed check has hitted
            )
            ...
            acc_avg_all_user_train = sum(acc_train_collect_user) / len(
                acc_train_collect_user
            )
            loss_avg_all_user_train = sum(loss_train_collect_user) / len(
                loss_train_collect_user
            )
    ...
    with torch.no_grad():
        ...
        if count2 == len_batch:
            ...
            if fed_check:
                ...
                acc_avg_all_user = sum(acc_test_collect_user) / len(
                    acc_test_collect_user
                )
                loss_avg_all_user = sum(loss_test_collect_user) / len(
                    loss_test_collect_user
                )
                ...
                print("====================== SERVER V1==========================")
                print(
                    " Train: Round {:3d}, Avg Accuracy {:.3f} | Avg Loss {:.3f}".format(
                        ell, acc_avg_all_user_train, loss_avg_all_user_train
                    )
                )
                print(
                    " Test: Round {:3d}, Avg Accuracy {:.3f} | Avg Loss {:.3f}".format(
...
class Client(object):
    def __init__(
        ...
        self.ldr_train = DataLoader(
            DatasetSplit(dataset_train, idxs), batch_size=256 * 4, shuffle=True
        )
        self.ldr_test = DataLoader(
            DatasetSplit(dataset_test, idxs_test), batch_size=256 * 4, shuffle=True
        )
    ...
    def train(self, net):
        ...
        for iter in range(self.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                ...
                dfx = train_server(
                    client_fx, labels, iter, self.local_ep, self.idx, len_batch
                )
